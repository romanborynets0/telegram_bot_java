...
public class Main extends TelegramLongPollingBot {
    ...
    public String getBotToken() {
        return "6001244732:AAGv6kd0QLFiih5zPX6xnUPjfzyYahMHunw";
    }
    ...
    public void onUpdateReceived(Update update) {
        ...
        if (update.getCallbackQuery().getData().equals("level_3_task") && getLevel(chatId) == 3){
            ...
            message.setChatId(chatId);
            attachButtons(message, Map.of(
                    "Купити Джавелін (50 монет)", "by_javelin"
            ));

            sendApiMethodAsync(message);
        }
        if (update.getCallbackQuery().getData().equals("by_javelin") && getLevel(chatId) == 4){
